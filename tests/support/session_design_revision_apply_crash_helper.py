"""Controlled Session Design Build crash boundary for recovery integration tests."""
...
async def apply_then_exit(worker_id: str) -> int:
    """Commit Build state, then deliberately exit before orchestration completion."""
    ...
    try:
        ...
        if (
            claim is None
            or claim.step.step_type is not StepType.BUILD_SESSION_PLAN_REVISION
        ):
            return 1
...
if __name__ == "__main__":
    raise SystemExit(
        asyncio.run(
            apply_then_exit(
                sys.argv[1] if len(sys.argv) > 1 else "session-design-build-crash"
            )
        )
    )
