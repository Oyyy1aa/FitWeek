"""Session Design Plan application Run creation, confirmation, and cancellation."""
...
from app.application.errors import (
    ...
    SessionDesignRevisionConfirmationConflict,
)
...
_RUN_NAMESPACE = UUID("5f677d9d-d4be-432e-b7b5-b483b093a73f")
_STEP_NAMESPACE = UUID("1c9f2cab-a70f-4561-a25f-9daa86bbe5c7")


@dataclass(frozen=True, slots=True, kw_only=True)
class SessionDesignApplicationRunCreation:
    run: PlanningRun
    created: bool


class SessionDesignApplicationRunService:
    def __init__(
        self,
        *,
    ...
    async def create_run(
        ...
        *,
    ...
    ) -> SessionDesignApplicationRunCreation:
        ...
        if draft is None:
            raise RunNotFound("Session Design Draft was not found for this user.")
        run_id = uuid5(_RUN_NAMESPACE, f"{user.id}:{request_id}")
        payload: JsonObject = {
            ...
            "apply_client_request_id": f"session-design-run:{run_id}",
        }
        ...
        canonical = json.dumps(
            {"user_id": str(user.id), "input": request_fingerprint_payload},
            sort_keys=True,
            separators=(",", ":"),
        )
        ...
        step = AgentStep(
            id=uuid5(_STEP_NAMESPACE, f"{run.id}:1"),
            run_id=run.id,
        ...
        return SessionDesignApplicationRunCreation(
    ...
    async def get_run(self, user: UserAccount, run_id: UUID) -> PlanningRun:
        ...
        if (
            run is None
            or run.workflow_type is not WorkflowType.SESSION_DESIGN_PLAN_INTEGRATION
        ):
            raise RunNotFound("Session Design application Run was not found.")
        return run
        ...
        *,
        ...
        if run.status not in {
            ...
            raise RunNotWaitingConfirmation(
                "Session Design application Run is not waiting for confirmation."
            )
        waiting = await self._waiting_step(run.id)
        if waiting.status is AgentStepStatus.SUCCEEDED:
            return run
        payload = dict(waiting.output_payload or waiting.input_payload)
        try:
            root_plan_id = UUID(str(payload["root_plan_id"]))
            created_revision = int(str(payload["created_revision"]))
            resulting_version = int(str(payload["resulting_plan_version"]))
        except (KeyError, TypeError, ValueError) as exc:
            raise SessionDesignRevisionConfirmationConflict(
                "The confirmation checkpoint is incomplete."
            ) from exc
        if (
            ...
            raise SessionDesignRevisionConfirmationConflict(
                "The Revision or Plan version confirmation is stale."
            )
        target = await self._revisions.get_revision(
            user, root_plan_id, created_revision
        )
        if target.status is WeeklyPlanStatus.VALIDATED:
            target = await self._revisions.confirm_revision(
                user,
                root_plan_id,
                created_revision,
            ...
            target.status is not WeeklyPlanStatus.CONFIRMED
            or target.version != expected_plan_version + 1
            or not await self._revisions.is_current_revision(user, target)
        ):
            raise SessionDesignRevisionConfirmationConflict(
                "The target Revision cannot be confirmed by this Run."
            )
        resume_payload: JsonObject = {
            **payload,
            "confirmed_plan_version": target.version,
        }
        try:
            await self._repository.resume_waiting_step(
                run_id=run.id,
                expected_step_id=waiting.id,
                handler_version="phase-5b-user-plan-confirmation-v1",
                next_step_type=StepType.FINALIZE_SESSION_APPLICATION,
                now=self._clock.now(),
                resume_payload=resume_payload,
            )
        except CheckpointConflict as exc:
            raise SessionDesignRevisionConfirmationConflict(str(exc)) from exc
    ...
    async def _waiting_step(self, run_id: UUID) -> AgentStep:
        matches = [
            ...
            if item.step_type is StepType.WAIT_FOR_PLAN_REVISION_CONFIRMATION
            and item.status in {AgentStepStatus.WAITING_USER, AgentStepStatus.SUCCEEDED}
        ]
        if len(matches) != 1:
            raise RunNotWaitingConfirmation(
                "Run has no single Plan Revision confirmation checkpoint."
            )
        return matches[0]
