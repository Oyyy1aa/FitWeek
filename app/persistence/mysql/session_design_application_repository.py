"""Atomic MySQL commit for Session Design Draft applications."""
...
class MySQLSessionDesignApplicationRepository:
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(SessionDesignApplicationResultModel).where(
                    SessionDesignApplicationResultModel.id == str(result_id),
    ...
    async def get_result_by_request(
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(SessionDesignApplicationResultModel).where(
                    SessionDesignApplicationResultModel.user_id == str(user_id),
                    SessionDesignApplicationResultModel.client_request_id
                    == client_request_id,
                )
            )
            return None if row is None else self._result_from_row(row)
    ...
    async def get_result_by_draft(
        ...
        async with self._sessions() as session:
            row = await session.scalar(
                select(SessionDesignApplicationResultModel).where(
                    SessionDesignApplicationResultModel.user_id == str(user_id),
                    SessionDesignApplicationResultModel.draft_id == str(draft_id),
                )
            )
            return None if row is None else self._result_from_row(row)
            ...
            try:
                async with session.begin():
                    existing = await session.scalar(
                        select(SessionDesignApplicationResultModel)
                        .where(
                            SessionDesignApplicationResultModel.user_id
                            == str(result.user_id),
                            SessionDesignApplicationResultModel.client_request_id
                    ...
                    if existing is not None:
                        if (
                            ...
                            or existing.draft_id != str(result.draft_id)
                        ):
                            raise RepositoryUniqueError(
                                "session_design_application.user_request",
                                request_key,
                            )
                    ...
                    draft = await session.scalar(
                        ...
                        .where(
                            SessionDesignDraftModel.id == str(expected_draft.id),
                            SessionDesignDraftModel.user_id == str(result.user_id),
                        )
                        .with_for_update()
                    )
                    if draft is None:
                    ...
                    duplicate_draft = await session.scalar(
                        select(SessionDesignApplicationResultModel.id).where(
                            SessionDesignApplicationResultModel.draft_id
                            == str(expected_draft.id)
                        )
                    )
                    if duplicate_draft is not None:
                        raise RepositoryUniqueError(
                            "session_design_application.draft",
                            expected_draft.id,
                        )
                    ...
                    session.add(
                        AuditEventModel(
                            ...
                            step_id=None,
                            sequence_no=None,
                            event_type="SESSION_DESIGN_DRAFT_APPLIED",
                            event_metadata={
    ...
    async def clear(self) -> None:
        async with self._sessions() as session:
            async with session.begin():
                ...
                await session.execute(delete(SessionDesignApplicationResultModel))

    @staticmethod
    def _result_row(
        value: SessionDesignApplicationResult,
    ) -> SessionDesignApplicationResultModel:
        return SessionDesignApplicationResultModel(
            id=str(value.id),
            ...
            created_at=MySQLSessionDesignApplicationRepository._db_time(
                value.created_at
            ),
        )
    ...
    def _result_from_row(
        cls, row: SessionDesignApplicationResultModel
    ) -> SessionDesignApplicationResult:
        return SessionDesignApplicationResult(
            id=UUID(row.id),
            ...
            target_session_id=UUID(row.target_session_id),
    ...
    def _apply_draft(
        cls,
        row: SessionDesignDraftModel,
        value: SessionDesignDraft,
    ) -> None:
        ...
        row.applied_revision = value.applied_revision
        row.applied_session_id = (
            None if value.applied_session_id is None else str(value.applied_session_id)
        )
