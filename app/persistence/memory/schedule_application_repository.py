"""Atomic process-local commit for Schedule Draft, Revision, and result."""
...
class InMemoryScheduleApplicationRepository:
    ...
    async def get_result(
        ...
        async with self._store.lock:
            value = self._store._schedule_application_results.get(result_id)
    ...
    async def get_result_by_request(
        ...
        async with self._store.lock:
            result_id = self._store._schedule_application_id_by_request.get(
                (user_id, client_request_id)
            )
            return (
                deepcopy(self._store._schedule_application_results[result_id])
    ...
    async def get_result_by_draft(
        ...
        async with self._store.lock:
            result_id = self._store._schedule_application_id_by_draft.get(draft_id)
            if result_id is None:
                return None
            result = self._store._schedule_application_results[result_id]
    ...
    async def commit(
        ...
        *,
        ...
        revision_key = (revision.user_id, revision.week_start, revision.revision)
        async with self._store.lock:
            existing_id = self._store._schedule_application_id_by_request.get(
                request_key
            )
            if existing_id is not None:
                existing = self._store._schedule_application_results[existing_id]
                if existing.application_fingerprint != result.application_fingerprint:
                    raise RepositoryUniqueError(
                        "schedule_application.user_request", request_key
                    )
                ...
                return ScheduleApplicationCommit(
                    ...
                    draft_id=existing.schedule_draft_id,
                    created=False,
                )
            current_draft = self._store._schedule_drafts.get(expected_draft.id)
            if (
                ...
                raise RepositoryConflictError(
                    ...
                    actual_version=(
                        -1 if current_draft is None else current_draft.version
                    ),
                )
            ...
            if stored_source is None or stored_source.version != source.version:
                raise RepositoryConflictError(
                    ...
                    actual_version=(
                        -1 if stored_source is None else stored_source.version
                    ),
                )
            confirmed = sorted(
                (
            ...
            if expected_draft.id in self._store._schedule_application_id_by_draft:
                raise RepositoryUniqueError(
                    "schedule_application.draft", expected_draft.id
                )
            if self._fail_next_commit:
                self._fail_next_commit = False
                raise RuntimeError("Injected atomic Schedule application failure.")
            ...
            self._store._schedule_application_results[result_snapshot.id] = (
                result_snapshot
            )
            self._store._schedule_application_id_by_request[request_key] = (
                result_snapshot.id
            )
            self._store._schedule_application_id_by_draft[expected_draft.id] = (
                result_snapshot.id
            )
            self._store._schedule_drafts[expected_draft.id] = deepcopy(applied_draft)
            return ScheduleApplicationCommit(
